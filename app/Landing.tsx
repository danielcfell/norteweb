/* ============================================================
   app/Landing.tsx — Server Component (sin "use client").
   Landing SEO-first para 04 Tech. Contenido crítico (precios,
   servicios, zonas, teléfono, FAQ) como texto HTML real.
   Lee de lib/seo.ts (misma fuente que metadata y JSON-LD).
   Interacción sin JS: FAQ con <details>, nav con anchors.
   Imágenes: next/image (preload solo en el hero).
   Jerarquía: 1 H1 → H2 por sección → H3 en tarjetas y FAQ.
   Paleta bloqueada: crema #E9E4D6 · tinta #16302A ·
   verde #2F6B4F · ocre #C9772F.
   ============================================================ */
...
/* ---------- mockups SVG (data-URI), misma paleta, dimensiones 560x420 ---------- */
...
const MOCK_BOT = `data:image/svg+xml,${encodeURIComponent(
  `<svg xmlns='http://www.w3.org/2000/svg' width='560' height='420' viewBox='0 0 560 420'>
    ...
    <g transform='translate(180 26)'>
      ...
      <rect x='8' y='8' width='184' height='44' fill='${T.verde}'/>
...
const MOCKS: Record<string, { src: string; alt: string }> = {
  "pagina-web": { src: MOCK_WEB, alt: "Mockup de una página web de negocio en la pantalla de una laptop, hecha en Tulcán" },
  "tienda-en-linea": { src: MOCK_WEB, alt: "Mockup de una tienda en línea en la pantalla de una laptop" },
  "sistema-facturacion": { src: MOCK_SISTEMA, alt: "Pantalla de un sistema de facturación con reportes y tabla de facturas" },
  "soporte": { src: MOCK_SISTEMA, alt: "Pantalla de panel de soporte y mejoras del sistema" },
  "bot-whatsapp": { src: MOCK_BOT, alt: "Captura de un chat de WhatsApp donde el bot responde a un cliente y agenda una cita" },
};

/* ---------- CSS (hovers, focus-visible, responsive) ---------- */
const css = `
  html { scroll-behavior: smooth; }
  .fx a:focus-visible, .fx summary:focus-visible { outline: 3px solid ${T.ocre}; outline-offset: 3px; border-radius: 4px; }
...
export default function Landing() {
  const sueltos = SERVICIOS.filter((s) => s.id !== "bot-whatsapp");
  const bot = SERVICIOS.find((s) => s.id === "bot-whatsapp")!;
  ...
  return (
    <div className="fx" style={{ fontFamily: SANS, background: T.crema, color: T.tinta }}>
      ...
      <header style={{ position: "relative", overflow: "hidden", background: T.verdeNoche, color: T.cremaTx }}>
        ...
        <div style={{ position: "relative", zIndex: 2, maxWidth: 1200, margin: "0 auto", padding: "22px clamp(20px, 4vw, 44px) clamp(30px, 4vw, 44px)", display: "flex", flexDirection: "column", minHeight: "100svh" }}>
          <nav aria-label="Principal" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, paddingBottom: 18, borderBottom: "1px solid rgba(241,236,223,0.20)" }}>
            <a href="#inicio" aria-label="04 Tech — inicio" style={{ display: "inline-flex", alignItems: "center", gap: 10, color: T.cremaTx }}>
            ...
            <div className="nv-center" style={{ display: "flex", alignItems: "center", gap: 30, fontSize: 15, fontWeight: 500 }}>
              ...
              <a href="#bot-whatsapp" className="nv-link">Bot de WhatsApp</a>
              <a href="#zonas" className="nv-link">Zonas</a>
              <a href="#precios" className="nv-link">Precios</a>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
              <a href="#preguntas-frecuentes" className="nv-link nv-contacto" style={{ fontSize: 15, fontWeight: 500 }}>Preguntas</a>
          ...
          <div id="inicio" className="hero-inner" style={{ flex: 1, display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: "clamp(32px, 5vw, 72px)", alignItems: "center", padding: "clamp(40px, 6vh, 80px) 0" }}>
            <div style={{ maxWidth: 640 }}>
              <span style={{ display: "inline-flex", alignItems: "center", gap: 9, fontSize: 13.5, fontWeight: 600, color: T.cremaTx, background: "rgba(22,48,42,0.4)", border: "1px solid rgba(241,236,223,0.28)", borderRadius: 8, padding: "7px 14px", marginBottom: 24 }}>
              ...
              <h1 className="h1" style={{ margin: "0 0 22px", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(44px, 5.8vw, 78px)", lineHeight: 1.0, letterSpacing: "-0.035em", color: T.cremaTx }}>
                Página web en Tulcán para conseguir{" "}
                <span style={{ color: T.ocreClaro }}>más clientes por WhatsApp.</span>
              </h1>
              ...
              <p style={{ margin: "0 0 34px", fontSize: "clamp(17px, 1.5vw, 20px)", lineHeight: 1.6, color: "rgba(241,236,223,0.92)", maxWidth: "48ch", textWrap: "pretty" }}>
                En 04 Tech creamos tu <strong style={{ color: T.cremaTx, fontWeight: 600 }}>página web en Tulcán</strong>, tu{" "}
                <strong style={{ color: T.cremaTx, fontWeight: 600 }}>sistema de facturación</strong> y tu{" "}
                <strong style={{ color: T.cremaTx, fontWeight: 600 }}>bot de WhatsApp</strong> para que te encuentren en Google
                y te escriban en todo el Carchi. Desde ${sueltos[0].price}.
              </p>
            ...
            <div className="hero-media" style={{ position: "relative" }}>
              <Image
                src={MOCK_WEB}
                alt="Mockup de una página web de negocio en la pantalla de una laptop, hecha en Tulcán"
                width={560}
                height={420}
                preload
                fetchPriority="high"
                ...
                style={{ width: "100%", height: "auto", borderRadius: 18, boxShadow: "0 30px 60px rgba(0,0,0,0.34)", border: "1px solid rgba(241,236,223,0.14)" }}
              />
            </div>
          </div>
          ...
          <div className="hero-shelf" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 0, paddingTop: 24, borderTop: "1px solid rgba(241,236,223,0.22)" }}>
            {[
              ...
              <div key={s.t} style={{ paddingLeft: i === 0 ? 0 : "clamp(18px,2.5vw,32px)", borderLeft: i === 0 ? "none" : "1px solid rgba(241,236,223,0.16)" }}>
                <p style={{ margin: "0 0 5px", fontFamily: DISPLAY, fontWeight: 700, fontSize: 18, letterSpacing: "-0.02em", color: T.cremaTx }}>{s.t}</p>
      ...
      <main>
        ...
        <section style={{ padding: `${pad} 0`, background: T.crema }}>
          <div className="man-grid" style={{ ...wrap, display: "grid", gridTemplateColumns: "1.1fr 0.9fr", gap: "clamp(36px, 6vw, 88px)", alignItems: "center" }}>
            <div>
              ...
              <h2 style={{ margin: "18px 0 0", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(30px, 4.2vw, 50px)", lineHeight: 1.04, letterSpacing: "-0.02em", color: T.tinta, textWrap: "balance" }}>
                Si no te encuentran en internet, le compran a otro
              </h2>
              <p style={{ margin: "20px 0 26px", fontSize: "clamp(16.5px, 1.4vw, 19px)", lineHeight: 1.66, color: T.tintaSuave, maxWidth: "54ch", textWrap: "pretty" }}>
                ...
                aparece, ese cliente se va a la competencia. Nosotros hacemos que te
                encuentren, te escriban y te compren — con tu página web, tu sistema
                de facturación y tu bot de WhatsApp.
              </p>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              {[
                ...
                <div key={v.t} style={{ background: T.cremaClaro, border: `1px solid ${T.borde}`, borderRadius: 14, padding: "18px 20px", display: "flex", gap: 16, alignItems: "flex-start" }}>
                  ...
                  <div>
                    <p style={{ margin: "0 0 3px", fontFamily: DISPLAY, fontWeight: 700, fontSize: 20, letterSpacing: "-0.02em", color: T.tinta }}>{v.t}</p>
          ...
          <div style={wrap}>
            <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 24, flexWrap: "wrap", marginBottom: "clamp(32px, 4vw, 56px)" }}>
              <div style={{ maxWidth: 640 }}>
                ...
                <h2 style={{ margin: "16px 0 0", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(30px, 4.2vw, 50px)", lineHeight: 1.04, letterSpacing: "-0.02em", color: T.tinta, textWrap: "balance" }}>
                  Páginas web, sistemas y bots para tu negocio en el Carchi
                </h2>
              </div>
            ...
            <div className="svc-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "clamp(16px, 2vw, 22px)" }}>
              {sueltos.map((s) => {
                const m = MOCKS[s.id];
                return (
                  <article key={s.id} id={s.id} className="svc" style={{ background: T.crema, border: `1px solid ${T.borde}`, borderRadius: 18, padding: "clamp(20px, 2.2vw, 26px)", overflow: "hidden", scrollMarginTop: 90 }}>
                    <Image src={m.src} alt={m.alt} width={560} height={420} unoptimized loading="lazy" sizes="(max-width: 560px) 90vw, 45vw" style={{ width: "100%", height: "auto", borderRadius: 12, marginBottom: 20, border: `1px solid ${T.borde}` }} />
                    <h3 style={{ margin: "0 0 10px", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(22px, 2.3vw, 27px)", letterSpacing: "-0.025em", color: T.tinta }}>{s.h3}</h3>
              ...
              {/* bot: tarjeta ancha destacada */}
              <article id={bot.id} className="svc svc-wide" style={{ gridColumn: "1 / -1", background: T.crema, border: `1.5px solid ${T.ocre}`, borderRadius: 18, padding: "clamp(24px, 2.6vw, 34px)", overflow: "hidden", scrollMarginTop: 90 }}>
                <div className="svc-wide-grid" style={{ display: "grid", gridTemplateColumns: "1.05fr 0.95fr", gap: "clamp(24px, 3vw, 48px)", alignItems: "center" }}>
                  <div>
                    ...
                    <h3 style={{ margin: "0 0 12px", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(24px, 2.6vw, 31px)", letterSpacing: "-0.025em", color: T.tinta }}>{bot.h3}</h3>
                    <p style={{ margin: "0 0 18px", fontSize: 16, lineHeight: 1.62, color: T.tintaSuave, textWrap: "pretty" }}>{bot.desc}</p>
                    <p style={{ margin: "0 0 18px", paddingTop: 14, borderTop: `1px solid ${T.bordeSuave}`, fontSize: 13.5, fontWeight: 600, color: T.verde }}>{bot.para}</p>
                    <a href={waLink(bot.waMsg)} className="tlink">Quiero mi bot de WhatsApp →</a>
                  </div>
                  <div className="svc-wide-media">
                    <Image src={MOCKS[bot.id].src} alt={MOCKS[bot.id].alt} width={560} height={420} unoptimized loading="lazy" sizes="(max-width: 760px) 90vw, 45vw" style={{ width: "100%", height: "auto", borderRadius: 14, border: `1px solid ${T.borde}`, boxShadow: "0 16px 34px rgba(22,48,42,0.16)" }} />
                  </div>
                </div>
              </article>
            </div>
        ...
        <section id="proceso" style={{ padding: `${pad} 0`, background: T.verde, color: T.cremaTx }}>
          <div style={wrap}>
            <div style={{ maxWidth: 640, marginBottom: "clamp(32px, 4vw, 56px)" }}>
              ...
              <h2 style={{ margin: "16px 0 0", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(30px, 4.2vw, 50px)", lineHeight: 1.04, letterSpacing: "-0.02em", color: T.cremaTx, textWrap: "balance" }}>
                Empezar es tan fácil como mandar un mensaje
              </h2>
        ...
        <section id="zonas" style={{ background: T.verdeNoche, color: T.cremaTx }}>
          <div className="zonas-grid" style={{ maxWidth: 1400, margin: "0 auto", display: "grid", gridTemplateColumns: "1fr 1fr", alignItems: "stretch" }}>
            <div style={{ padding: `${pad} clamp(20px, 4vw, 60px)`, display: "flex", flexDirection: "column", justifyContent: "center" }}>
              ...
              <h2 style={{ margin: "18px 0 0", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(30px, 4.2vw, 50px)", lineHeight: 1.04, letterSpacing: "-0.02em", color: T.cremaTx, textWrap: "balance" }}>
                Página web en Tulcán, Julio Andrade y todo el Carchi
              </h2>
              <p style={{ margin: "20px 0 30px", fontSize: 17, lineHeight: 1.65, color: T.humo, maxWidth: "46ch", textWrap: "pretty" }}>
                Trabajamos con negocios de toda la provincia del Carchi. Estés donde
                estés, coordinamos por WhatsApp o videollamada — y cuando el proyecto
                lo amerita, vamos hasta donde estás.
              </p>
              <ul style={{ listStyle: "none", margin: "0 0 30px", padding: 0, display: "flex", flexWrap: "wrap", gap: 10 }}>
                {AREAS.map((canton) => (
                  <li key={canton} className="chip" style={{ display: "inline-flex", alignItems: "baseline", gap: 8, border: "1px solid rgba(241,236,223,0.28)", borderRadius: 9, padding: "9px 16px", fontSize: 15, fontWeight: 500, color: T.cremaTx, background: "rgba(241,236,223,0.05)" }}>
                    {canton}
                    {canton === "Tulcán" && <span style={{ fontSize: 13, color: T.ocreClaro }}>· nuestra casa</span>}
                  </li>
                ))}
              </ul>
            ...
            <div className="zonas-img" style={{ position: "relative", minHeight: 480, overflow: "hidden", background: T.verde, display: "grid", placeItems: "center", padding: "clamp(28px,4vw,56px)" }}>
              ...
              <Image src={MOCK_BOT} alt="Captura de un chat de WhatsApp donde el bot de WhatsApp responde a un cliente" width={560} height={420} unoptimized loading="lazy" sizes="(max-width: 900px) 78vw, 40vw" style={{ position: "relative", width: "min(78%, 360px)", height: "auto", borderRadius: 20, boxShadow: "0 26px 54px rgba(0,0,0,0.36)" }} />
            </div>
          </div>
        </section>

        {/* ================= PRECIOS ================= */}
        <section id="precios" style={{ padding: `${pad} 0`, background: T.crema, scrollMarginTop: 70 }}>
          <div style={wrap}>
            <div style={{ maxWidth: 680, marginBottom: "clamp(32px, 4vw, 52px)" }}>
              <Kicker bg={T.verde}>Precios</Kicker>
              <h2 style={{ margin: "16px 0 0", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(30px, 4.2vw, 50px)", lineHeight: 1.04, letterSpacing: "-0.02em", color: T.tinta, textWrap: "balance" }}>
                Precios de página web y sistemas en el Carchi
              </h2>
              <p style={{ margin: "18px 0 0", fontSize: 17, lineHeight: 1.6, color: T.tintaSuave, textWrap: "pretty" }}>
                Cada servicio cuesta ${PLANES[0].price}. ¿Quieres los tres? El
                paquete completo es ${PAQUETE.price} — te ahorras plata y tu negocio
            ...
            <div className="plan-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "clamp(16px, 2vw, 24px)", alignItems: "stretch", marginBottom: "clamp(18px, 2vw, 24px)" }}>
              {PLANES.map((p) => (
                <article key={p.id} className="plan" style={{ display: "flex", flexDirection: "column", background: T.cremaClaro, color: T.tinta, border: `1px solid ${T.borde}`, borderRadius: 18, padding: "clamp(24px, 2.4vw, 32px)" }}>
                  ...
                  <p style={{ margin: "0 0 3px", display: "flex", alignItems: "baseline", gap: 8 }}>
                    <span style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(38px, 3.4vw, 48px)", letterSpacing: "-0.03em", lineHeight: 1, color: T.ocre }}>${p.price}</span>
                  ...
                  <a href={waLink(p.waMsg)} className="plan-cta plan-cta-line" style={{ display: "inline-flex", justifyContent: "center", alignItems: "center", gap: 9, padding: "13px 20px", borderRadius: 11, fontSize: 15.5, fontWeight: 700, background: "transparent", color: T.tinta, border: `1.5px solid ${T.tinta}` }}>
            ...
            <article className="plan paquete-grid" style={{ display: "grid", gridTemplateColumns: "1.15fr 0.85fr", gap: "clamp(24px, 3vw, 48px)", background: T.verdeNoche, color: T.cremaTx, borderRadius: 20, padding: "clamp(28px, 3vw, 44px)", alignItems: "center", position: "relative", overflow: "hidden" }}>
              ...
              <div style={{ position: "relative" }}>
                ...
                <p style={{ margin: "0 0 6px", display: "flex", alignItems: "baseline", gap: 10 }}>
                  <span style={{ fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(48px, 5vw, 68px)", letterSpacing: "-0.04em", lineHeight: 1, color: T.ocreClaro }}>${PAQUETE.price}</span>
                ...
                <a href={waLink(PAQUETE.waMsg)} className="btn-ocre" style={{ display: "inline-flex", alignItems: "center", gap: 11, padding: "16px 32px", borderRadius: 11, fontSize: 17, fontWeight: 700, boxShadow: "0 14px 30px rgba(201,119,47,0.36)" }}>
                  <WaIcon />
                  Quiero el paquete de ${PAQUETE.price}
                </a>
              </div>
        ...
        {/* ================= FAQ ================= */}
        <section id="preguntas-frecuentes" style={{ padding: `${pad} 0`, background: T.cremaOsc, borderTop: `1px solid ${T.bordeSuave}`, scrollMarginTop: 70 }}>
          <div style={{ ...wrap, maxWidth: 880 }}>
            <div style={{ marginBottom: "clamp(28px, 4vw, 48px)" }}>
              <Kicker bg={T.verde}>Preguntas frecuentes</Kicker>
              <h2 style={{ margin: "16px 0 0", fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(30px, 4.2vw, 50px)", lineHeight: 1.04, letterSpacing: "-0.02em", color: T.tinta, textWrap: "balance" }}>
                Preguntas frecuentes sobre páginas web y bots en el Carchi
              </h2>
            </div>
              ...
              {FAQS.map((f) => (
                <details key={f.q} className="faq" style={{ borderBottom: `1px solid ${T.borde}` }}>
                  <summary style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 20, padding: "22px 4px" }}>
                    <h3 className="faq-q" style={{ margin: 0, fontFamily: DISPLAY, fontWeight: 700, fontSize: "clamp(18px, 2vw, 22px)", letterSpacing: "-0.02em", color: T.tinta }}>{f.q}</h3>
        ...
        <section id="contacto" style={{ padding: `${pad} 0`, background: T.ocre, color: "#FFF7EC", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ ...wrap, position: "relative", textAlign: "center", maxWidth: 820 }}>
            <h2 style={{ margin: 0, fontFamily: DISPLAY, fontWeight: 800, fontSize: "clamp(34px, 4.8vw, 58px)", lineHeight: 1.04, letterSpacing: "-0.02em", color: "#FFF7EC", textWrap: "balance" }}>
              Empieza a conseguir clientes esta semana
            </h2>
            <p style={{ margin: "20px auto 36px", fontSize: "clamp(16.5px, 1.5vw, 19px)", lineHeight: 1.64, color: "rgba(255,247,236,0.94)", maxWidth: "50ch", textWrap: "pretty" }}>
              ...
              sistema de facturación o bot de WhatsApp, desde ${PLANES[0].price}.
            </p>
            ...
            <p style={{ margin: "20px 0 0", fontSize: 14, color: "rgba(255,247,236,0.85)" }}>
              O visítanos en {BUSINESS.city} — coordinamos la cita por WhatsApp.
            </p>
          </div>
        </section>
      </main>

      {/* ================= FOOTER (NAP idéntico a schema/metadata) ================= */}
      <footer style={{ background: T.verdeNoche, color: "rgba(241,236,223,0.62)" }}>
        <div className="foot-grid" style={{ ...wrap, padding: "clamp(48px, 6vw, 72px) clamp(20px, 4vw, 44px) 26px", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: "clamp(28px, 4vw, 60px)" }}>
          ...
          <nav aria-label="Footer" style={{ display: "flex", flexDirection: "column", gap: 12, fontSize: 15 }}>
            ...
            <a href="#bot-whatsapp" className="footlink">Bot de WhatsApp</a>
            <a href="#zonas" className="footlink">Zonas</a>
            <a href="#precios" className="footlink">Precios</a>
            <a href="#preguntas-frecuentes" className="footlink">Preguntas frecuentes</a>
          </nav>
          <div style={{ display: "flex", flexDirection: "column", gap: 12, fontSize: 15 }}>
            ...
            <a href={waLink("Hola 04 Tech, quiero información")} className="footlink">WhatsApp: {BUSINESS.phoneE164}</a>
        ...
        <div style={wrap}>
          <div style={{ borderTop: "1px solid rgba(241,236,223,0.12)", padding: "18px 0 24px", display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "space-between", fontSize: 12.5, color: "rgba(241,236,223,0.44)" }}>
            <p style={{ margin: 0 }}>© {new Date().getFullYear()} {BUSINESS.name} · {BUSINESS.city}, {BUSINESS.region}</p>
            <p style={{ margin: 0 }}>Diseño web · sistemas de facturación · bots de WhatsApp en el Carchi</p>
          </div>
        </div>
      </footer>
    </div>
